    ...
    public function supplierPurchase($supplier_id)
    {
        $purchases = Purchase::with('supplier')
            ...
            ->map(function ($purchase) {
                ...
                $paymentStatus = $purchase->paid_amount >= $purchase->grand_total ? 'Paid' : ($purchase->paid_amount > 0 ? 'Partial' : 'Due');
        ...
        if (!empty($sales)) {
            ...
            foreach ($sales as $key => $sale) {
                ...
                if (in_array("sales-delete", $request['all_permission']))
                    $nestedData['options'] .= '<form action="' . route('sales.destroy', $sale->id) . '" method="POST" style="display:inline;">
                        ' . csrf_field() . '
                        ' . method_field("DELETE") . '
                            <li>
                              <button type="submit" class="btn btn-link" onclick="return confirmDelete()"><i class="dripicons-trash"></i> ' . __("db.delete") . '</button>
