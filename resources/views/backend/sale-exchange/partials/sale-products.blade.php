...
if (Auth::user()->role_id <= 2) {
    ...
    {{-- Exchange Checkbox --}}
    {{--
            FIX: value is now $product_sale->id (the unique product_sales row ID)
            instead of $product->code.

            WHY: When the same product appears multiple times in one invoice with
            different prices, all rows share the same product code. Using the code
            as the checkbox value meant checking ONE row would accidentally mark ALL
            rows of that product for return. Using product_sale->id is unique per
            row, so each checkbox controls exactly the sale line it belongs to.
        --}}
